# `test1_band_power.py` — kod do niezależnej oceny

Wersja bliźniacza skryptu z Testu 1, wystawiona jako dokument zgodnie z pętlą
roboczą (`CPS_OPERATING_BRIEF.md` §5). Ten plik nie jest wykonywany — służy
przeglądowi przed uruchomieniem.

**Realizuje:** `TEST1_PROTOCOL.md` v1.0
**Zależności:** numpy, scipy, pandas, matplotlib. Świadomie **bez statsmodels** —
AR estymowany metodą Yule'a–Walkera, żeby wynik nie zależał od zawartości środowiska.
**Czas wykonania:** ok. 50 s dla B = 2000 (18 uruchomień), ok. 4 min dla B = 10000.
**Wersja:** poprawiona po przeglądzie Code'a — dodany panel 3 (rozkłady zerowe P1/P2),
uzupełniony panel 1 (serie po detrendingu) i kolumna `decyzja` w CSV. §10 realizowany w całości.

## Na co zwrócić uwagę przy przeglądzie

1. `prepare()` nakłada ten sam filtr i detrending na dane **i na każdy surogat** —
   to jest zabezpieczenie przed efektem Slutsky'ego–Yule'a i nie może zostać pominięte.
2. Model zerowy estymowany jest na serii po detrendingu (`base`), a nie na surowej.
3. `psd()` stosuje okno Hanna i uzupełnienie zerami ×4 — zagęszcza próbkowanie
   widma, **nie** zwiększa rozdzielczości.
4. `fold_chi2()` liczy fazę od pierwszego roku analizowanego okna, nie od roku 0.
   Wartości χ² nie są więc porównywalne z liczbami z sesji lutowej, które używały
   `rok mod T`, ani z wartościami liczonymi na serii wygładzonej.
5. `build_specs()` zawiera pre-rejestrowaną listę 18 uruchomień z §7 protokołu.
   **Nie dodawaj i nie usuwaj pozycji.**
6. Reguła decyzyjna w `main()` odtwarza §8 protokołu dosłownie; orzeka wyłącznie P1.
