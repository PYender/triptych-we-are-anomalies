# `test4_robustness.py` — kod do niezależnej oceny (Etap A)

Wersja bliźniacza skryptu Testu 4 (rodzina 2, kontrola H2). Ten plik nie jest wykonywany.

**Realizuje:** `TEST4_PROTOCOL.md` v1.0 (D-010) · **Status:** Etap A — do przeglądu, **NIEURUCHOMIONY**.
**Zależności:** numpy, pandas, matplotlib + import `test1_band_power` (fit_ar_yw, sim_ar, linear_detrend).
Test 4 czyta gotowy `cps_canonical_v2.csv` (builder v2.0 wystarcza).

## Konstrukcja
Statystyka **M2 = χ²(1914–2007) − χ²(1816–1913)**, epoch-folding, 10 koszy fazowych.
Null AR(3) o rzędzie z góry, B = 2000, ziarno 20260814. Orzeka wyłącznie **P1**
(krzywa M2(k), k=0…10, `A_COW_P`, T=35,1). Warianty: P2 (R1, wojny światowe), S1 (R2
winsoryzacja 95/90), S2 (R3 przy T=32/40), S3 (R1/R2/R3 na `A_COW_W`), S4 (pasmo
odniesienia z usuwania losowego, 500 powt.), **S5** (kontrola — patrz niżej).

## Sześć punktów z §A2 — jak są rozwiązane
1. **Faza z numeru roku, nie z pozycji** — `fold_chi2_years()` liczy fazę jako
   `((rok − początek_okna) mod T)`; usunięcie lat nie przesuwa fazy pozostałych.
   Dla okna ciągłego redukuje się do `fold_chi2` z Testu 1 (zgodność z M2 z Testu 3).
2. **Detrending raz, na całej serii** (D-004) — `detrend_mode="global"`: detrend przed
   podziałem i przed usuwaniem lat.
3. **To samo przetwarzanie na surogatach** — surogat detrendowany i przechodzi tę samą
   transformację (usunięcie lat / winsoryzacja) co obserwacja.
4. **R3 dla surogatów: własne k najwyższych** — każdy surogat zdejmuje swoje k najwyższych
   lat epoki 2, nie te same lata kalendarzowe.
5. **Winsoryzacja osobno dla każdej epoki**, każda własnym percentylem.
6. **S4 osobny strumień losowy** (`S4_SEED = SEED+1`) niż surogaty.

## Rozstrzygnięcia autora naniesione
- **Ranking „k najwyższych" po detrendingu** — R3 usuwa najwyższe lata serii, na której
  liczone jest M2 (po detrendingu globalnym). Raport poda **obie** listy: surową (D-010,
  opis historyczny) i po detrendingu (usuwaną przez R3); różnica z dodatniego trendu serii
  uczestniczej (detrending ściąga późne szczyty mocniej niż wczesne).
- **S5 — kontrola raportowana OBOK, poza regułą §6.** `detrend_mode="epoch"` (detrending
  wewnątrz epoki) dla obu serii; odpowiada na pytanie o trend wewnątrz epoki postawione
  w przeglądzie. §2 pozostaje w mocy: P1 używa detrendingu globalnego, niezmiennego.

## Reguła decyzyjna (§6) i punkty do potwierdzenia
`decide()`: P1 pozytywny ⟺ M2>0 i p<0,05 przy k=5 **oraz** spadek M2 (k0→k5) mieści się
w oczekiwanym z S4 ±2σ. Do potwierdzenia w przeglądzie:
- baza AR fitowana zawsze na serii detrendowanej globalnie (stacjonarnej), także dla S5 —
  żeby AR(3) był dobrze uwarunkowany; obserwacja i surogat i tak kończą epoch-detrendem.
- p jednostronne (H2: M2>0), jak w Teście 3.
