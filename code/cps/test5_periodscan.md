# `test5_periodscan.py` — kod do niezależnej oceny (Etap A)

Wersja bliźniacza skryptu Testu 5 (rodzina 1 rozszerzona). Ten plik nie jest wykonywany.

**Realizuje:** `TEST5_PROTOCOL.md` v1.0 (D-011) · **Status:** Etap A — do przeglądu, **NIEURUCHOMIONY**.
**Zależności:** numpy, pandas, matplotlib + import `test1_band_power` (psd, band_share,
fit_ar_yw, sim_ar, linear_detrend, prepare) i `test4_robustness` (fold_chi2_years).
Czyta gotowy `cps_canonical_v2.csv`. **Szac. czas biegu:** 30–60 min (Etap B).

## Konstrukcja
Skan **T ∈ [8, 60], krok 0,5 — 105 punktów**. Statystyki: **W1** (udział mocy w paśmie
T±4, mianownik 4–100 lat) i **W2** (χ² epoch-folding, 10 koszy, faza z numeru roku).
Dla każdej liczone **Wmax** i **Targmax** po skanie. Null AR(3), B=2000, ziarno 20260815;
**rozkład zerowy to rozkład maksimów**: p = (1 + #{Wmax_sur ≥ Wmax_obs}) / (B+1).

## Sześć punktów z §A2 — jak są rozwiązane
1. **Rozkład zerowy = rozkład maksimów.** `run_config`: każdy surogat przechodzi pełny
   skan (`scan_W1`/`scan_W2`), bierzemy jego `max(axis=1)`; p z rozkładu tych maksimów.
2. **Pasmo W1 przesuwa się z T** (`T±4` w `w1_from_spectrum`); mianownik 4–100 lat stały.
3. **Detrending raz na oknie**, przed skanem, dla danych i surogatów (`prepare(...,det=True)`,
   baza AR = `linear_detrend`).
4. **Jeden zestaw surogatów na konfigurację** — jedna realizacja `sim_ar` → cały skan 105 pkt.
5. **Targmax surogatów** zbierany (`sur_targmax`) i rysowany (panel 3, kontrola jednorodności).
6. **S5 (MA(11))** — `prepare(y, ma=True, det=True)` nakłada filtr także na każdy surogat.

## Wydajność (A4)
`psd` liczone **raz na realizację**; wszystkie 105 pasm W1 odczytywane z gotowego widma
(widmo nie zależy od T). W2 nie da się tak zoptymalizować — faza zależy od T.

## Uruchomienia (§6)
P1 (A_COW_W 1816–2007, W1, orzeka) · P2 (W2) · S1 (A_COW_P) · S2 (A_COW_W 1914–2007) ·
S3 (A_COW_P 1914–2007) · S4 (B_UCDP) · S5 (A_COW_W, MA(11), W1). Reguła §7: P1 p<0,05
i P2 p<0,10 oraz rozkład Targmax surogatów nieskupiony wąsko — ostatni warunek to ocena
w raporcie, kod go nie orzeka progiem.

## Do potwierdzenia w przeglądzie
- Baza AR fitowana na serii detrendowanej (stacjonarnej); obserwacja i surogat przechodzą
  ten sam `prepare` (dla S5 z MA). Spójne z Testami 1/3/4.
- Jeden strumień `rng` (ziarno 20260815) na cały bieg; P1/P2 współdzielą realizacje
  surogatów A_COW_W (ten sam null AR(3)) — po jednym `run_config` na konfigurację serii/okna.
